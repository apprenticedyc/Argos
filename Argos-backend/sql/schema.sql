...
COMMENT ON COLUMN assistant_session_contexts.compact_summary             IS 'LLM 生成的压缩摘要（更精炼的压缩）';
...
COMMENT ON COLUMN assistant_session_contexts.compact_summary_base_message_id     IS '压缩摘要覆盖的起始消息 ID';
COMMENT ON COLUMN assistant_session_contexts.compact_summary_range_end_message_id IS '压缩摘要覆盖的结束消息 ID';
