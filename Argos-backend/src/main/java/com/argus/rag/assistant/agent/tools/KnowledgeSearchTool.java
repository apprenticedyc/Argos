...
import com.argus.rag.assistant.model.vo.tool.KnowledgeSearchToolResponse;
...
public class KnowledgeSearchTool {
    ...
    public static final String RESULT_HOLDER_CONTEXT_KEY = "knowledgeSearchToolResultHolder";
    ...
    public KnowledgeSearchTool(QaRetrievalService qaRetrievalService) {
     ...
     * <p>检索结果通过 {@link KnowledgeSearchToolResultCache} 记录引用列表，
     * 供 {@link AgentFacade} 在构建最终响应时使用。</p>
     *
    ...
    public KnowledgeSearchToolResponse search(
            @ToolParam(description = "用于检索知识库的查询文本，通常使用用户当前问题。") String query,
            ToolContext toolContext
    ) {
        Long groupId = readGroupId(toolContext);
        KnowledgeSearchToolResultCache resultHolder = readResultHolder(toolContext);
    ...
    private KnowledgeSearchToolResultCache readResultHolder(ToolContext toolContext) {
        Object value = toolContext.getContext().get(RESULT_HOLDER_CONTEXT_KEY);
        if (value instanceof KnowledgeSearchToolResultCache holder) {
            return holder;
        }
        throw new BusinessException("知识库检索缺少结果上下文");
    }

    private KnowledgeSearchToolResponse.Evidence toEvidence(Document document) {
        Map<String, Object> metadata = document.getMetadata();
        return new KnowledgeSearchToolResponse.Evidence(
