...
在你的项目中，`AssistantMemoryPromptConfiguration` 注册了三个提示词模板 Bean，分别用于**更新会话记忆**、**生成压缩摘要**和**运行时压缩上下文**——这些都是短时记忆系统的核心，后面会详细展开。
...
sequenceDiagram
    ...
    loop 大模型逐字生成
        ...
        Service-->>Controller: 发射 SSE 事件 "chunk"
        Controller-->>Browser: data: {"event":"chunk","chunk":"新字"}
    end
...
`SseEmitter` 是 Spring MVC 提供的 SSE（Server-Sent Events）基础设施，它允许服务器在 HTTP 连接上持续推送数据。控制器创建了一个 `SseEmitter` 实例，然后把它交给 `AssistantService.streamChat()` 方法。每当大模型生成一个新的片段（chunk），服务层就通过 `AssistantStreamEventEmitter.emit()` 回调发送一个 SSE 事件给浏览器。这种设计的巧妙之处在于：控制器不需要知道大模型怎么生成内容，只需要知道"有内容来了就推送给前端"。
...
flowchart TD
    ...
    B --> B1{工具模式为 KB_SEARCH?}
    B1 -- 是 --> B2[校验必填groupId<br/>校验用户为对应组成员]
    B1 -- 否 --> C
    B2 --> C[步骤2: 保存用户消息至数据库]
    C --> D[步骤3: 记忆维护-前置处理]
    D --> E[步骤4: 调用Agent生成对话回复]
    E --> F{Agent回复是否为空?}
    F -- 是 --> G[返回服务正忙兜底话术]
    F -- 否 --> H[步骤5: 保存助手回复至数据库]
    H --> I[步骤6: 记忆维护-后置处理]
    I --> J[步骤7: 组装结果响应返回Controller]
```
...
| `loadConversationContext()` | 加载完整的对话上下文（摘要 + 压缩摘要 + 会话记忆 + 最近消息） |
...
AssistantAgentResult streamChat(String instruction, RunnableConfig config,
    String userMessage, Consumer<String> chunkConsumer, ...)
```
...
- **处理流式输出的去重**：Spring AI Alibaba 的流式节点偶尔会返回累积文本（完整文本）而不是纯增量（只有新 token），Facade 通过 `normalizeStreamingchunk()` 方法去重——比较本次文本和上一次的文本，提取出真正新增的部分。
...
**③ 压缩摘要（Compact Summary）—— "长期压缩记忆"**

当会话记忆也越积越多时，系统再对它们进行二次压缩，生成"压缩摘要"。这是最简洁的形式，只覆盖**用户目标、关键进展、关键结论和未解决问题**。
...
| `compact-message-trigger` | 6 | 每新增 6 条消息，触发压缩摘要更新 |
...
flowchart LR
    ...
    subgraph "session-compact-summary.st"
        CS_IN[现有压缩摘要 + 会话记忆 + 待压缩消息] --> CS_OUT[新的压缩摘要]
    end

    subgraph "runtime-compact-summary.st"
        RC_IN[压缩摘要 + 会话记忆 + 最近消息 + 当前问题] --> RC_OUT[运行时压缩上下文]
    end
```
...
- **session-compact-summary.st**：当压缩摘要需要更新时，把现有的压缩摘要、当前的会话记忆、以及需要压缩的消息一起发给大模型，生成新的压缩摘要。
...
erDiagram
    ...
    assistant_session_contexts {
        ...
        text compact_summary "LLM 生成的压缩摘要"
        bigint session_memory_base_message_id "会话记忆覆盖的消息起始 ID"
        bigint session_memory_range_end_message_id "会话记忆覆盖的消息结束 ID"
        bigint compact_summary_base_message_id "压缩摘要覆盖的消息起始 ID"
        bigint compact_summary_range_end_message_id "压缩摘要覆盖的消息结束 ID"
...
sequenceDiagram
    ...
    Ctrl->>Svc: streamChat(request, chunkConsumer)
    Note over Svc: 校验、保存用户消息（和同步模式一样）

    Svc->>Facade: streamChat(instruction, config, message, chunkConsumer)

    loop 大模型流式生成
        Agent->>LLM: 发送请求 (stream=true)
        LLM-->>Agent: 返回 token 片段
        Facade->>Facade: normalizeStreamingchunk() 去重
        Facade-->>Svc: chunkConsumer.accept(chunk)
        Svc-->>Ctrl: emitter.send(AssistantChatStreamEvent.chunk(chunk))
        Ctrl-->>User: SSE data: {"event":"chunk","chunk":"token"}
    end
...
- **三级记忆结构**（最近消息 + 会话记忆 + 压缩摘要）：这比官方的"简单取最近 N 条消息"要精细得多。
