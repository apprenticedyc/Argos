...
流式模式在 Agent 执行阶段额外插入 SSE 事件推送（start → chunk × N → done）。
...
sequenceDiagram
    ...
    rect rgb(255, 248, 240)
        ...
        Memory->>Memory: 判断是否触发会话记忆/压缩摘要更新
        alt 触发阈值
...
流式聊天在阶段 3-6 之间插入 SSE 事件推送（设计决策 [4.4](#44-sse-流式聊天与-chunk-去重)）。
...
| `AssistantConversationContext` | `AssistantConversationService` | `AssistantShortTermMemoryHook` | `{prunedHistory, compactSummary, sessionMemory, recentMessages}` |
...
## 4.4 SSE 流式聊天与 chunk 去重

### 为什么需要 chunk 去重

Spring AI Alibaba Agent Framework 的流式实现通过 Reactor `Flux<NodeOutput>` 推送图节点输出。在 `AGENT_MODEL_STREAMING` 阶段，框架会推送模型生成的文本。

但在某些模型后端（如 DashScope）下，流式节点的 `message.getText()` 返回的不是**增量 chunk**，而是**截至当前的全文**——即每次推送的都是"从开头到当前位置的完整文本"。如果直接透传给前端，前端会看到重复的前缀。

### 解决方案：前缀减法

```java
private String normalizeStreamingchunk(String candidateText, String accumulatedReply) {
    if (candidateText == null || candidateText.isBlank()) {
        return "";
    }
    if (accumulatedReply.isEmpty()) {
        return candidateText;  // 第一条 chunk，直接使用
    }
...
`accumulatedReply` 是一个 `StringBuilder`，在整次流式调用中持续累积——每次收到新文本，裁掉已累积的前缀，只将真正的增量交给 `chunkConsumer`。

### 流式处理的两种输出类型

```java
private void handleStreamingOutput(NodeOutput output, Consumer<String> chunkConsumer, StringBuilder finalReply) {
    ...
    if (type == OutputType.AGENT_MODEL_STREAMING) {
        // 正常流式路径：模型边生成边推送
        String chunk = normalizeStreamingchunk(assistantMessage.getText(), finalReply.toString());
    ...
    if (type == OutputType.AGENT_MODEL_FINISHED
        ...
        chunkConsumer.accept(fullText);
    }
}
```

`AGENT_MODEL_FINISHED` 兜底路径处理的是模型不走逐字流式输出的情况——此时 `finalReply` 为空，需要将完整文本作为一次性 chunk 发送。
